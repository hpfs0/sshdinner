...
(function () {
    ...
    var J = window.J || window.jQuery,
        findNearest = function (el, tagName) {
...
window.CONFIG = {
    DEFAULT_LOGO: 'http://' + DOMAINS.IMGCACHE + '/qzonestyle/xiaoyou_portal_v1/img/0.gif'
};
window.FBrowser = {};
...
window.G_V = {
    "pTitle": "温馨提示",
    "bimg": "http://qzonestyle.gtimg.cn/campus_v2/ac/b.gif"
};
...
window.getCookieUserInfo = function (cb, force) {
    ...
    var hash = J.cookie("hash"),
        name = J.cookie("name"),
        sex = J.cookie("sex"),
        appint = J.cookie("appint");
    if (!isEmpty(hash)) {
...
window.global_frame_new = function (title, html, option) {
    ...
    else FsetOffsetWindowPostionByRate(w, 2, 4); if (op.no_modal == false) {
...
window.XY_APP = (function () {
    var urls = {
        ...
        allDataBase = {}, userDataBase = {}, lastUAList = {}, lastAAList = {}, canvasUrlPool = {};
...
window.doSendPoke = function (u, id, type) {
    ...
    var cb = function () {
        ...
        if (data.getElementsByTagName("error").length > 0) {
            ...
            if (er.getAttribute("type") == "login") html_error_frame('温馨提示', "您已登录超时，请重新登录");
            else {
                if (er.textContent) html_error_frame('温馨提示', er.textContent);
                else html_error_frame('温馨提示', er.text);
            }
